...
app.post('/address/',(req,res)=>{
	...
	// })
		...
		ChainSchema.find( { $or: [ { "transactions.receiver": req.body.search }, { "transactions.sender": req.body.search } ] },(err, docs)=>{
		var tx=[];
		if(docs.length>0){
			for(i=0;i<docs.length;i++){
				tx.push(docs[i].transactions.filter((y)=>y.sender==req.body.search || y.receiver==req.body.search)) 
			}   
